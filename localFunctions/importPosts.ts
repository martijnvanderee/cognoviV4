...
export const importPost = async (path: string): Promise<PostData> =>
  await import(`../content/posts/${path}`);
...
const getSubsetPosts = (files: string[], numberOfPosts: number) =>
  files.slice(0, numberOfPosts);

export const getPosts = async (
  AMOUNT_OF_POST_FRONTPAGE: number,
  sortSubject: string = "all"
): Promise<PostData[]> => {
  const postsSortedBySubject =
    sortSubject === "all"
      ? data.FileNames
      : postMeta.postPerSubject[sortSubject];

  const sortedSlugsByData = sortPostsByDate(postsSortedBySubject);

  const slugs = await getSubsetPosts(
    sortedSlugsByData,
...
export const getFullPost = async (slug: string) => {
  ...
  const p1: any = photos.photosData
    ? photos
    : {
        headerData: photos.headerData,
        photosData: [],
      };
...
const test = async (photosData: any) =>
  photosData ? photosData.length !== 0 && importPhotos(photosData) : [];

export const getSpecificPhoto = async ({
  headerData,
  photosData,
}: SpecificPhotos) => ({
  headerData: await importPhoto1(headerData),
  photosData: await test(photosData),
});

const getNumberOfPages1 = (subjectName: string) =>
  convertNumberToArray(getNumberOfPages(subjectName));

const getSubjectPath = (subjectName: string) =>
  getNumberOfPages1(subjectName).map((arr: number) => ({
    params: {
      subject: subjectName,
      id: (arr + 1).toString(),
    },
...
const getSpecificPosts = (slugs: string[], num: Num) =>
  num.start > slugs.length
    ? slugs.slice(num.start)
    : slugs.slice(num.start, num.end);
...
const triple = (array: any) => view1(array);
...
export const getRandomPostBySubject = async (
  amountOfPosts: number,
  subject: string,
  postMeta: PostMeta
) => {
  const paths = getRandomPaths(
    amountOfPosts,
    postMeta.postPerSubject[subject].length,
    postMeta.postPerSubject[subject]
  );
...
export const getRandomPosts = async (
  numberOfPost: number = 1
): Promise<FullPost[]> => {
  const paths = getRandomPaths(
    numberOfPost,
    postMeta.amountOfPosts,
    postMeta.FileNames
  );
...
export const getNumberOfPages = (subject: string) =>
  Math.ceil(getNumberOfPosts(subject, postMeta) / NUMBER_OF_POST_ON_PAGE);

const getNumberOfPosts = (subject: string, postMeta: PostMeta) =>
  subject === "all"
    ? postMeta.amountOfPosts
    : postMeta.amountOfPostPerSubject[subject];
