        ...
        <div className="relative w-full md:max-w-4xl md:mt-10 md:mx-auto mb-4">
          ...
          <div className="relative -top-10 md:-top-12 w-full m-auto">
            <div className="sm:w-9/12 mb-4 m-auto">
              <div className="z-10 text-grey text-small text-sm mt-0 pm">
                bron: {titleBron}
              </div>
              <div className="z-10 text-gray-800  text-lg font-medium italic m-0">
                {alt}
              </div>
            </div>
          </div>
      ...
      <main className="md:max-w-6xl  md:mx-auto bg-white">
        <div className="relative w-full h-72 md:max-w-4xl md:h-96 md:pt-10 md:mx-auto">
          ...
          <div className="md:w-8/12 m-auto mb-4">
            <p className="z-10 text-grey text-small text-sm">
              bron: {photos.headerData.bron}
            </p>
          </div>
        </div>
        ...
        <div className="p-4 md:mb-4">
          <div className=" mb-6 mt-4 md:mt-8 md:max-w-xl md:mx-auto">
            <header>
              <h1 className="text-3xl mb-2 font-bold text-black md:text-4xl md:max-w-xl md:mx-auto">
                {title}
              </h1>
              {subtitle && (
                <h2 className="italic text-xl text-black mb-4 md:text-2xl md:max-w-xl md:mx-auto">
                  {subtitle}
                </h2>
              )}
            </header>
          ...
          {photos.photosData.length !== 0 && (
            <div className="relative w-full md:max-w-4xl md:mt-10 md:mx-auto mb-4">
              ...
              <div className="w-full m-auto">
                <div className="sm:w-9/12 mb-4 m-auto">
                  <p className="z-10 text-grey text-small text-sm">
                    bron: {photos.photosData[0].bron}
                  </p>

                  <p className="z-10 text-gray-800  text-lg font-medium italic">
                    {photos.photosData[0].onderschrift}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="mx-2">
          {tags && (
            <ul className="flex flex-wrap mb-6 gap-y-2 mb-20">
              {tags.map((tag: string) => {
                return (
                  <li className="text-purple font-bold p-2 border-2 border-purple rounded-md mr-4 whitespace-nowrap">
                    {tag}
                  </li>
                );
              })}
...
export async function getStaticProps({ params }: params) {
  ...
  const randomPosts: FullPost[] = await getRandomPostBySubject(
    4,
    post.onderwerp,
    postMeta
  );
